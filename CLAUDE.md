...
**`scripts/atualizar_resultados.py`** script standalone que consulta a API `football-data.org` (competição `WC`) e atualiza `gols_mandante`/`gols_visitante`/`penaltis_mandante`/`penaltis_visitante` na aba `jogos` para jogos finalizados no dia. Requer `FOOTBALL_DATA_TOKEN` no `.env`. As colunas de pênaltis só são gravadas quando o jogo foi a pênaltis (`score.penalties` não nulo na API). O dicionário `NOMES_TIMES` dentro do script mapeia nomes em inglês (API) para português (planilha) — se aparecer `[MISS]` nos logs, adicionar a entrada faltante lá. A comparação de data usa UTC (igual à API). **Janela dupla de datas:** nas 00h–02h UTC (quando o cron ainda corre pós-meia-noite), o script consulta também o dia anterior UTC (`dateFrom=ontem`) para capturar jogos iniciados às 19h+ BRT (= 22h+ UTC) que terminam após meia-noite UTC — eles ficam registrados na API com a data do dia anterior.
...
- O `ACCESS_TOKEN` da Meta expira em 24h em ambiente de teste. Sintoma: erro `401 OAuthException code 190` nos logs. **Mesmo tokens marcados como `#permanente` no `.env` expiram** (visto em produção — token gerado como permanente expirou ~1 mês depois). Se o bot parar de responder (nenhum template chega ao usuário), testar o token diretamente antes de investigar outra coisa: `curl "https://graph.facebook.com/v20.0/$PHONE_NUMBER_ID?access_token=$ACCESS_TOKEN"` — erro `code 190` confirma expiração. Gerar novo token no painel da Meta, atualizar `.env` local, copiar para a VPS via `scp` e reiniciar o serviço.
- O usuário `deploy` na VPS tem `sudo` sem senha (`NOPASSWD`) para comandos como `systemctl restart bolao` — usar `sudo -n systemctl restart bolao` via SSH não deve travar aguardando senha interativa. Já `journalctl -u bolao` e logs do nginx (`/var/log/nginx/`) exigem senha interativa de sudo (não fazem parte do NOPASSWD) — para esses, pedir ao usuário para rodar localmente com `!`.
...
- **`id_jogo` com espaços:** células da aba `jogos` podem ter espaços no final do `id_jogo` (ex: `'20260617PORRD  '`). Todo código que compara ou armazena `id_jogo` deve usar `.strip()` — já aplicado em `sheets/detalhe_jogo.py`, `sheets/aposta_automatica.py`, `sheets/apostas.py` e `handlers/aposta.py`. Se surgir bug de jogo sumindo no apostas-do-dia ou apostas automáticas duplicadas, verificar primeiro a normalização do `id_jogo`.
